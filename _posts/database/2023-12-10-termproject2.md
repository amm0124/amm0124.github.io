...
**즉, 다시 말하자면 각 개별 물품에 대해서 product_code를 부여하고, 이를 primary key로 사용하자. 그리고 상위 product_code로 묶자. 그러면 공간도 덜 소모할 것이고 깔끔할 것이다.** 
...
[product_details] 생성 쿼리

    create TABLE product_details(
        top_product_code VARCHAR primary key,
...
[code_product_mapping] 생성 쿼리
    ...
    CREATE TABLE code_mapping_table (
        subcode VARCHAR,
        topcode VARCHAR,
        FOREIGN KEY (topcode) REFERENCES product_details (top_product_code) ON DELETE CASCADE ON UPDATE CASCADE
    );
...
[product_table] 생성 쿼리
    ...
    CREATE TABLE product_table (
        ...
        product_price INT,
        product_count INT,
        FOREIGN KEY (subcode) REFERENCES code_product_mapping (subcode) ON DELETE CASCADE ON UPDATE CASCADE
    );
