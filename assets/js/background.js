...
var particleCount=40,flareCount=16,motion=.05,tilt=.05,color="#44b5fd",particleSizeBase=2,particleSizeMultiplier=.5,flareSizeBase=100,flareSizeMultiplier=100,lineWidth=1,linkChance=75,linkLengthMin=5,linkLengthMax=7,linkOpacity=.375,linkFade=90,linkSpeed=1,glareAngle=-60,glareOpacityMultiplier=.05,renderParticles=!0,renderParticleGlare=!0,renderFlares=!0,renderLinks=!0,renderMesh=!1,flicker=!0,flickerSmoothing=15,blurSize=0,orbitTilt=!0,randomMotion=!0,noiseLength=1e3,noiseStrength=1;
var canvas=document.getElementById("stars"),context=canvas.getContext("2d"),mouse={x:0,y:0},m={},r=0,c=1e3,n=0,nAngle=2*Math.PI/noiseLength,nRad=100,nScale=.5,nPos={x:0,y:0},points=[],vertices=[],triangles=[],links=[],particles=[],flares=[],EPSILON=1/1048576;

function randomColor() {
    return color;  // Use a single color for all elements
}

function supertriangle(t){var e,i,s,o,a,l,h=Number.POSITIVE_INFINITY,$=Number.POSITIVE_INFINITY,f=Number.NEGATIVE_INFINITY,d=Number.NEGATIVE_INFINITY;for(e=t.length;e--;)t[e][0]<h&&(h=t[e][0]),t[e][0]>f&&(f=t[e][0]),t[e][1]<$&&($=t[e][1]),t[e][1]>d&&(d=t[e][1]);return o=Math.max(i=f-h,s=d-$),[[(a=h+.5*i)-20*o,(l=$+.5*s)-o],[a,l+20*o],[a+20*o,l-o]]}

function circumcircle(t,e,i,s){var o,a,l,h,$,f,d,u,v,p,g=t[e][0],_=t[e][1],x=t[i][0],k=t[i][1],y=t[s][0],P=t[s][1],I=Math.abs(_-k),b=Math.abs(k-P);if(I<EPSILON&&b<EPSILON)throw Error("Eek! Coincident points!");return I<EPSILON?(h=-((y-x)/(P-k)),f=(x+y)/2,u=(k+P)/2,a=h*((o=(x+g)/2)-f)+u):b<EPSILON?(l=-((x-g)/(k-_)),$=(g+x)/2,d=(_+k)/2,a=l*((o=(y+x)/2)-$)+d):(l=-((x-g)/(k-_)),h=-((y-x)/(P-k)),$=(g+x)/2,f=(x+y)/2,d=(_+k)/2,o=(l*$-h*f+(u=(k+P)/2)-d)/(l-h),a=I>b?l*(o-$)+d:h*(o-f)+u),v=x-o,p=k-a,{i:e,j:i,k:s,x:o,y:a,r:v*v+p*p}}

function dedup(t){var e,i,s,o,a,l;for(i=t.length;i;)for(o=t[--i],s=t[--i],e=i;e;)if(l=t[--e],s===(a=t[--e])&&o===l||s===l&&o===a){t.splice(i,2),t.splice(e,2);break}}

function Delaunay(t,e){var i,s,o,a,l,h,$,f,d,u,v,p,g=t.length;if(g<3)return[];if(t=t.slice(0),e)for(i=g;i--;)t[i]=t[i][e];for(o=Array(g),i=g;i--;)o[i]=i;for(o.sort(function(e,i){return t[i][0]-t[e][0]}),a=supertriangle(t),t.push(a[0],a[1],a[2]),l=[circumcircle(t,g+0,g+1,g+2)],h=[],$=[],i=o.length;i--;$.length=0){for(p=o[i],s=l.length;s--;){if((f=t[p][0]-l[s].x)>0&&f*f>l[s].r){h.push(l[s]),l.splice(s,1);continue}f*f+(d=t[p][1]-l[s].y)*d-l[s].r>EPSILON||($.push(l[s].i,l[s].j,l[s].j,l[s].k,l[s].k,l[s].i),l.splice(s,1))}for(dedup($),s=$.length;s;)v=$[--s],u=$[--s],l.push(circumcircle(t,u,v,p))}for(i=l.length;i--;)h.push(l[i]);for(l.length=0,i=h.length;i--;)h[i].i<g&&h[i].j<g&&h[i].k<g&&l.push(h[i].i,h[i].j,h[i].k);return l}
...
function init(){
    ...
    for (let t = 0; t < particles.length; t++) {
        for (let e = 0; e < triangles.length; e++) {
            let i = triangles[e].indexOf(t);
            if (i !== -1) {
                triangles[e].forEach(function(e){
                    if (e !== t && particles[t].neighbors.indexOf(e) === -1) {
                        particles[t].neighbors.push(e);
                    }
                });
            }
        }
...
function render() {
    ...
    if (renderMesh) {
        ...
        for (let e = 0; e < vertices.length - 1; e++) {
            if ((e + 1) % 3 !== 0) {
                let i = particles[vertices[e]],
                    s = particles[vertices[e+1]],
    ...
    if (renderLinks) {
        if (random(0, linkChance) === linkChance) {
            let l = random(linkLengthMin, linkLengthMax);
            startLink(random(0, particles.length - 1), l);
        }
        for (let h = links.length - 1; h >= 0; h--) {
            if (links[h] && !links[h].finished) {
                links[h].render();
            } else {
                delete links[h];
            }
...
function noisePoint(t) {
    return {
        x: Math.cos(t * nAngle) * nRad * nScale,
        y: Math.sin(t * nAngle) * nRad * nScale
    };
}

function distance(t, e, i, s) {
...
backgroundImage.src = 'your-background-image-url.jpg'; // 여기에 배경 이미지 URL 입력
